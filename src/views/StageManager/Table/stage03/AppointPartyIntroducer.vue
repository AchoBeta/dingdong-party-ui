    ...
    <el-table :data="tableData" style="width: 100%" v-loading="loading">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <template>
            <el-button type="mini" @click="selectionIntroducer(scope.row)">指定入党介绍人</el-button>
            <!-- 联系人表格 -->
            <el-dialog title="可选介绍人" :visible.sync="dialogTableVisible">
              <!-- 表单 -->
              <el-form 
                :inline="true" 
                :model="selectForm">
                <el-input v-show="false" v-model="selectForm.recommend_first_id"></el-input>
                <el-input v-show="false" disabled v-model="selectForm.recommend_second_id"></el-input>
                <el-form-item label="第一入党介绍人(必填):">
                  <el-input disabled v-model="selectForm.firName" placeholder="从下方选择"></el-input>
                </el-form-item>
                <el-form-item label="第二入党介绍人(选填):">
                  <el-input disabled v-model="selectForm.secName" placeholder="从下方选择"></el-input>
                </el-form-item>
                <el-form-item>
                  <el-button type="primary" @click="selectFormSubmit">确定</el-button>
                </el-form-item>
              </el-form>
              <el-table 
                :data="introducerData"
                ...
                <el-table-column property="party_positions" label="党内职务" width="200"></el-table-column>
                <el-table-column property="administrative_position" label="行政职位"></el-table-column>
                <el-table-column property="party_age" label="党龄"></el-table-column>
              </el-table>
              <!-- 分页组件 -->
              <el-pagination
                background
  ...
  export default {
    ...
    data() {
      return {
        ...
        ,selectForm:{
          casid: '', // 学号
          recommend_first_id: "", // 第一培养联系人（选填）（若不填可以传0或不传）
          firName: '', // 第一培养联系人姓名 只作显示用
          recommend_second_id: "", // 第二培养联系人（选填）（若不填可以传0或不传）
    ...
    ,methods: {
      ...
      // 发送联系人表单
      selectFormSubmit(){
        if(this.selectForm.recommend_first_id == 0){
          this.$message({
            message: '至少选择一个联系人',
            type: 'error',
            duration: 1000
        ...
        //
        request({
          url: `assignRecommands`,
          params: {
            casid: this.selectForm.casid,
            recommend_first_id: this.selectForm.recommend_first_id,
            recommend_second_id: this.selectForm.recommend_second_id
          },
          method: 'post'
        }).then(res=>{
          this.$message({
      ...
      handleSelectionChange(val){
        if(val.length >= 2){
          this.selectForm.recommend_first_id = val[0].id
          this.selectForm.firName = val[0].name
          this.selectForm.recommend_second_id = val[1].id
          this.selectForm.secName = val[1].name
        }else if(val.length == 1){
          this.selectForm.recommend_first_id = val[0].id
          this.selectForm.firName = val[0].name
          this.selectForm.recommend_second_id = 0
          this.selectForm.secName = ''
        }else{
          this.selectForm.recommend_first_id = 0

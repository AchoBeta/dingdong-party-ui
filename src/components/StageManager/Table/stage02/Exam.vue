<template>
  <div>
    <el-table :data="tableData" style="width: 100%" v-loading="loading">
      ...
      <el-table-column
        label="成绩"
        prop="score.branch_school_score"
      ></el-table-column>
      <el-table-column label="操作">
        <template slot-scope="scope">
          <template>
            <el-button size="mini" @click="getCasid(scope.row)"
              >成绩录入</el-button
            >
            <el-popconfirm
              title="是否确认操作"
              @onConfirm="nextTask(scope.row)"
            >
            ...
            <el-dialog
              ...
              width="450px"
            >
              <el-form :model="gradeForm" :inline="true">
                <el-form-item label="成绩：">
                  <el-input
                    v-model="gradeForm.branch_school_score"
                    autocomplete="off"
                  ></el-input>
                </el-form-item>
                <el-button type="primary" @click="gradeSubmit()"
                  >确 定</el-button
                >
              </el-form>
            </el-dialog>
...
export default {
  ...
  methods: {
    ...
    gradeSubmit() {
      //判断成绩合法
      if (
        this.gradeForm.branch_school_score > 100 ||
        this.gradeForm.branch_school_score < 0 ||
        this.gradeForm.branch_school_score == ""
      ) {
        this.$message({
